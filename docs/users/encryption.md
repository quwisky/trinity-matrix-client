...
| You need to erase a broken installation                    | This removes all Trinity data on this device, not just the encryption backup. | Read [erase local data](signing-in.md#starting-over-when-trinity-will-not-work) and type `ERASE` only if you accept that broader loss. |
