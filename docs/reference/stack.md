...
| `@ng-icons/lucide`                    | 34.0.0  | Icon set behind `<trn-icon>`; imported by `@trinity/components/icon` and five generated kit libraries; `@ng-icons/core` moves with it                                                           |
...
| `blurhash`                            | 2.0.5   | Decodes the MSC2448 placeholder painted behind an image while it loads. MIT, no dependencies. Its `isBlurhashValid` checks length only, so the base83 alphabet is checked in `@trinity/util/ui` |
| `luxon`                               | 3.7.2   | Date and time arithmetic, including the day-separator rollover                                                                                                                                  |
| `@ctrl/ngx-emoji-mart`                | 9.3.0   | Emoji picker for the composer and reactions, reached only through `@trinity/components/emoji-picker`                                                                                            |
