...
Status: **planned** (Phase 0 spike validated on branch `spike/zoneless`, off `chore/nx-23.1.0`).
...
| **1 — Production switch + NgZone cleanup** (1 PR) | Bootstrap swap · remove all 18 NgZone wrappers/injects · fix comments · retire the 1 zone test · drop `zone.js` + unused `@angular/animations` | `nx build/lint/test` green (tests still zoned here); manual smoke of the risk register |
| **2 — Test harness zoneless** (1 PR)              | Swap ~21 setups + 58 ATL imports · resolve the ATL/zoneless input-timing issue · fix intricate specs                                           | `nx run-many -t test` green under zoneless                                             |
