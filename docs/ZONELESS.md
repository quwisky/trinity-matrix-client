...
Status: **Phase 1 done** — production runs zoneless with no NgZone (branch `spike/zoneless`,
commit `86693c6`). Phase 2 (test harness) remains, blocked on an ATL version issue; see below.
...
| **2 — Test harness zoneless** — ⚠️ blocked on ATL              | With the centralized `test-setup.base.ts` the setup flip is now **one file** (`setupTestBed({ zoneless: true })`); the 59 ATL specs still swap to `@testing-library/angular/zoneless`. **Blocker:** ATL **19.4.1** (the latest — no fix release) runs a component's initial CD _before_ binding programmatic `inputs`, throwing `NG0950` on required signal inputs. **Candidate fix:** a shared `render` wrapper that passes `autoDetectChanges: false` and runs `detectChanges()` after inputs bind, imported in place of ATL across the specs. Drops `zone.js` once green. | `nx run-many -t test` green under zoneless                        |
