...
| `libs/data-access/timeline`      | `@trinity/data-access/timeline`      | `type:data-access`, `scope:matrix`     | `ConversationRuntime` owns immutable Account-and-Room handles, durable compose intent, typed text/media send lifecycles and child timeline projections; Message Presentation normalizes events into immutable models and replaces media security material with opaque references; the library also contains `ThreadsService`, URL previews and edit history       |
