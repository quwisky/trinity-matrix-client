...
| `libs/feature/rooms`    | `@trinity/feature/rooms`    | `type:feature`, `scope:matrix` | The chat application surface: rooms shell, sidebar, server rail, message presentation and toolbar, composer, reactions, polls, media, members and space management; plus the page-scoped exact-Conversation lifecycle for threads, pins, in-Room search, reveal and semantic Back |
