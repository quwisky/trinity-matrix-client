...
| `libs/data-access/timeline`      | `@trinity/data-access/timeline`      | `type:data-access`, `scope:matrix`     | `ConversationRuntime` owns immutable Account-and-Room handles, durable compose intent, exact message relation/action commands, privacy-aware receipts, typed text/media send lifecycles and child timeline projections; Message Presentation normalizes SDK-authoritative events into immutable models and replaces media security material with opaque references; Room Administration supplies redaction governance through a composition-root port; the library also contains `ThreadsService`, URL previews and edit history |
