...
| `libs/kit/emoji-picker` | `@trinity/kit/emoji-picker` | `type:ui`, `scope:shared`, `ui:vendor-wrapper` | Trinity-authored: `<trn-emoji-picker>` over `TrnEmojiPick`, plus the `TrnEmojiIndex` facade the `:shortcode` autocomplete uses — the only importer of `@ctrl/ngx-emoji-mart`                                                                                        |
