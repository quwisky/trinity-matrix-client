...
| `libs/data-access/room-library`        | `@trinity/data-access/room-library`        | `type:data-access`, `scope:matrix`     | Room Library: one selected-Account view over joined Room and Space summaries plus Account-scoped invitations; exact ownership, active/mixed selection, shared-Room unread merge, shared-Space child union, capability-owned typed selection persistence, favourites, low-priority grouping, ordering, canonical named-row filtering and ranked local search, hierarchy, create/join/leave actions, compatibility mixed projections, and aggregate unread. Its one-shot mutations are cold finite Observables, including account-scope and ordering persistence, favourite/priority writes, hierarchy changes, unread cleanup, and the bounded exact-Account readiness barrier used before post-create or post-invite navigation                                                                                                                                        |
