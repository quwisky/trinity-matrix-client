...
| Library                          | Alias                                | Tags                                   | Purpose                                                                                                                                                                                                                                                                                                                                            |
| -------------------------------- | ------------------------------------ | -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `libs/data-access/accounts`      | `@trinity/data-access/accounts`      | `type:data-access`, `scope:matrix`     | Account Runtime: read-only lifecycle state plus cold restore, authenticated-establishment, and atomic Active Account switch commands; it prioritises the Active Account, bounds saved-Account recovery, joins identical operations, rejects conflicting transitions, and owns the post-commit projection-readiness barrier behind one adapter seam |
| `libs/data-access/matrix-client` | `@trinity/data-access/matrix-client` | `type:data-access`, **`scope:shared`** | The client and session foundation: `MatrixClientService` (a registry of concurrently syncing accounts), `SecretStorageKeyHolder`, the Matrix adapter for Projection Runtime's sync-state tracer, and `projectFromClient`, which registers existing active-client projections with Projection Runtime                                               |
| `libs/data-access/auth`          | `@trinity/data-access/auth`          | `type:data-access`, `scope:matrix`     | Password, legacy SSO, OIDC-native authentication and registration produce opaque Account grants; compatibility facades retain logout, `FactoryResetService`, and `authGuard` while lifecycle ownership moves into Account Runtime                                                                                                                  |
...
| `libs/data-access/media`         | `@trinity/data-access/media`         | `type:data-access`, `scope:matrix`     | `MediaService` for encrypted attachments, `AvatarService`, the bounded MSC2545 `ImagePackService` projection, `ImagePackManagementService`, and its short-lived server-confirmed `ImagePackSelectionStore`                                                                                                                                         |
...
| `libs/feature/settings` | `@trinity/feature/settings` | `type:feature`, `scope:matrix` | Exports `settingsRoutes` plus the lazy-loaded `SettingsDialogComponent`; both consume one internal registry of fourteen sections                                                                  |
