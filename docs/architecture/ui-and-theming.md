...
| `progress` · `HlmProgressIndicator`                          | Its indeterminate sweep is guarded for reduced motion in `theme/spartan.css` — the class is applied through a `[class.…]` binding, which cannot take a variant | `kit-reduced-motion.spec.mjs`   |
