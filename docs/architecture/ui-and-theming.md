...
| Semantic surfaces   | Component-facing aliases: `--trinity-surface-frame`, `-navigation`, `-navigation-header`, `-workspace`, `-raised`, `-floating`; `--trinity-border-subtle` / `-strong`. These point inward to the palette primitives, never the other way round.                                                                                                                                                                                                                                                                                                                                  |
| Interaction states  | Paired `--trinity-state-{hover,pressed,selected,selected-hover,attention}-{surface,foreground}` roles, plus `--trinity-focus-ring` / `-width` / `-offset` and `--trinity-disabled-opacity`. A state is a pair so palette tuning cannot change its fill without its ink.                                                                                                                                                                                                                                                                                                          |
...
| Spacing and density | `--trinity-space-1`…`-7` — a 4px rhythm (2, 4, 8, 12, 16, 24, 32). Components consume `--trinity-density-item-gap`, `-row-gap`, `-row-padding-*` and `-control-size`; Compact re-cuts these while coarse-pointer targets retain their global 44px floor.                                                                                                                                                                                                                                                                                                                         |
...
--ring: var(--trinity-focus-ring);
--input: var(--border);
```

The focus role deliberately resolves to the measured link colour rather than the accent fill.
The fill drops below the 3:1 non-text threshold on some dark interactive surfaces; the focus role
is checked against every semantic surface in every palette/mode combination.
