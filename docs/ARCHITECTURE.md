...
| [spaces.service.ts](../libs/core/src/lib/matrix/spaces.service.ts)                         | Spaces read model + write actions. `spaces` signal (joined `m.space` rooms) + `childRoomIds`; plus the writes behind the server rail / sidebar: `createSpace` (`m.space` creation), `createRoomInSpace` (an encrypted room linked via `m.space.child` + `m.space.parent`), and `leaveSpace`.                                                                                                                                                   |
...
- `send-media-run.mjs`, `threads-run.mjs`, `spaces-run.mjs` — full feature flows against
  the disposable Synapse: encrypted media send/receive; the thread lifecycle ("Reply in
  thread" → the first reply creates the thread, reopen, abandon-creates-nothing); and
  spaces create/manage (create a space, create a channel in it asserting the
  `m.space.child`/`m.space.parent` links + child encryption, leave). Each `*-run.mjs` owns
  the Synapse lifecycle, so they **must run sequentially** (one shared Docker stack on
  fixed ports), e.g. `pnpm e2e:threads && pnpm e2e:spaces`.
