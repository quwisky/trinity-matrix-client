...
1. **Leave a room** (S) — you literally can't exit a joined non-space room today.
2. **Redact others' messages as a moderator** (S) — cheapest safety win; gating change only.
3. **Room settings panel** (M) — name/topic/avatar; the container every other admin control hangs off.
...
6. **Per-room notification level** (M) — mute / mentions-only; also listed in the UX track.
...
pin, role **display** (Admin/Moderator/Member sections). What's missing is the ability to _act_.

| Feature                                              | Size | Value |
| ---------------------------------------------------- | ---- | ----- |
| Leave a normal room (+ Forget after leaving)         | S    | High  |
...
- **Leave a room (S):** the header/sidebar only offer `leaveSpace`/`declineInvite`; a joined regular room
  has no exit. Mirror the existing `defer/from(client.leave)` pattern; add Forget after leaving.
- **Redact others' (S):** deletion is hard-gated to `message.isOwn`. Expose the existing redact affordance
  when the user's power level meets the room's redact threshold (reuse the `maySendStateEvent` precedent
  from `PinnedMessagesService.canPin`). Highest safety-per-effort in the whole track.
- **Room settings panel (M):** no room-settings surface exists (`goToSettings` goes to _app_ settings).
  `setRoomName` / `setRoomTopic` / `sendStateEvent(m.room.avatar)` behind a power-level gate. This is the
  container that unblocks join-rules, aliases, ban-list, and power-level editing.
...
**Sequencing:** Leave + Redact-others (quick safety wins) → Room settings panel → Member info panel →
...
device management, GIF config, feature flags, presence **display**.
...
- **Per-room notification level (M):** the engine already respects push rules and the sidebar even renders a
  cosmetic "muted-style" badge — but there's no control to actually mute or set mentions-only. Table stakes
  in every mainstream client. `setRoomMutePushRule` / room push rules; account-data plumbing already exists.
...
For maximum "feels like Element" per week, start with the **safety + control quick wins**: Leave-a-room,
Redact-others, Per-room mute, and Ignore/block (all S–M, all High). Then build the two containers that
unblock the rest — the **Room settings panel** and the **Member info panel** — and layer moderation,
power-level editing, and cross-user verification on top.
