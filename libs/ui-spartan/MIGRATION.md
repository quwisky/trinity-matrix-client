...
| `ion-icon`                                                                                                                                                                | `<ng-icon>` (`@ng-icons/core`) + lucide, per-component `provideIcons`                                                                                                                                                                       | ✅ done |
| `ion-app`, `ion-router-outlet`, `ion-split-pane`, `ion-menu`, `ion-menu-button`, `ion-header`, `ion-toolbar`, `ion-title`, `ion-content`, `ion-footer`, `ion-back-button` | custom Tailwind **app shell** — `<div h-dvh flex-col>` + Angular `RouterOutlet` (global `router-outlet + *` page-fill rule), a responsive `<aside>` drawer, flex `<header>`/`<main>`, `env(safe-area-inset-*)`, Capacitor `App` back button | ✅ done |
