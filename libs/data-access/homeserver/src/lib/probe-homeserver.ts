 ...
 * promise looks identical and silently breaks re-subscription — a settled promise replays
 * its result, so a second subscribe would issue no request at all. That matters directly
 * here: "Check again" re-subscribes. Same trap documented at `@trinity/util/matrix`'s
 * `fetchMediaBytes`.
 */

/**
 * Per-request budget. Short on purpose: three of these run per account, the section is a
 * glance, and a homeserver that has not answered in this long is "unknown" for the reader's
 * purposes anyway. Without it a hung connection leaves the block spinning and the
...
export function probeServerSoftware(
  ...
  return fetchFederationVersion(baseUrl, 'base-url').pipe(
    switchMap((direct) =>
      direct ? of(direct) : probeDelegatedSoftware(serverName),
    ),
  );
}
...
function probeDelegatedSoftware(
  ...
  const wellKnown = safeUrl(`https://${serverName}/.well-known/matrix/server`);
  if (!wellKnown) {
    return of(null);
  }
  return getJson(wellKnown).pipe(
    switchMap((body) => {
      ...
      const origin = server ? federationOrigin(server) : null;
      return origin ? fetchFederationVersion(origin, 'delegated') : of(null);
    }),
  );
}
...
export function fetchSpecVersions(
  ...
  const url = safeUrl('/_matrix/client/versions', baseUrl);
  if (!url) {
    return of(null);
  }
 ...
 *
 * Raw for the same reason as {@link fetchSpecVersions}: `client.getCapabilities()` reads a
 * six-hour poller cache and the SDK exposes no way to force it, so it cannot answer
...
export function fetchCapabilities(
  ...
  const url = safeUrl('/_matrix/client/v3/capabilities', baseUrl);
  if (!url || !accessToken) {
    return of(null);
  }
  return getJson(url, accessToken).pipe(
 ...
 * *server* chose, so it is validated rather than coerced. Prefixing `https://` and trusting
