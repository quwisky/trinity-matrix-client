 ...
 * with dynamic `import('@trinity/feature/crypto')` calls, so this library never imports the
 * feature — keeping the `type:ui` (scope:shared) → `type:feature` (scope:matrix) module
 * boundary intact while {@link EncryptionDialogService} can still present those pages as
 * modals on desktop. Absent in tests with no app wiring, in which case the service always
