 ...
 * commented at its site and pinned by a test in libs/kit/overlay (the only spartan lib
 * with a Vitest target), so a lost override fails the suite rather than shipping.
 *
 *   1. TrnDropdownMenuSubTrigger — `_handleClick` shadowed so a sub-trigger click OPENS the
 *      submenu instead of toggling it closed under zoneless CD. See docs/architecture/state-and-reactivity.md.
 *   2. TrnDropdownMenuSubTrigger — the shadowed `_handleClick` also re-does CDK's focus move,
 *      so keyboard Enter/Space lands in the submenu.
 *   3. TrnDropdownMenuSubTrigger — `side` defaults to 'right' rather than the root-menu
 *      config, so a submenu opens BESIDE its parent instead of over it.
 *   4. TrnDropdownMenu / TrnDropdownMenuSub — CdkTargetMenuAim host directive, so travelling
 *      diagonally into an open submenu doesn't close it on the way.
 *   5. TrnDropdownMenuItem — a destructive item's TEXT and ICON use `text-danger`, not
 ...
 */
@Directive({
  selector: '[trnDropdownMenuFocusOnHover]',
  host: {
    '(mouseenter)': '_focusOnHover()',
  },
})
export class TrnDropdownMenuFocusOnHover {
...
export class TrnDropdownMenuCheckboxIndicator {
  constructor() {
    classes(
      () =>
...
/** @internal. Use TrnDropdownMenuCheckbox instead. */
@Directive({
  selector: '[trnDropdownMenuCheckboxCdk]',
  providers: [
    { provide: CdkMenuItemCheckbox, useExisting: TrnDropdownMenuCheckboxCdk },
    { provide: CdkMenuItemSelectable, useExisting: TrnDropdownMenuCheckboxCdk },
    { provide: CdkMenuItem, useExisting: CdkMenuItemSelectable },
  ],
})
export class TrnDropdownMenuCheckboxCdk extends CdkMenuItemCheckbox {
...
@Directive({
  selector: '[trnDropdownMenuCheckbox],[trnDropdownMenuCheckboxItem]',
  hostDirectives: [
    {
      directive: TrnDropdownMenuCheckboxCdk,
      inputs: ['cdkMenuItemDisabled: disabled', 'cdkMenuItemChecked: checked', 'keepOpen'],
      outputs: ['cdkMenuItemTriggered: triggered'],
    },
    { directive: TrnDropdownMenuFocusOnHover, inputs: [], outputs: [] },
  ],
  host: {
...
export class TrnDropdownMenuCheckbox {
  protected readonly _cdkMenuItem = inject(TrnDropdownMenuCheckboxCdk);
...
@Directive({
  selector: '[trnDropdownMenuGroup],trn-dropdown-menu-group',
  hostDirectives: [
    { directive: CdkMenuGroup, inputs: [], outputs: [] },
  ],
  host: { 'data-slot': 'dropdown-menu-group' },
})
export class TrnDropdownMenuGroup {
  constructor() {
    classes(() => 'block');
  }
}
...
export class TrnDropdownMenuItemSubIndicator {
  constructor() {
...
@Directive({
  selector: '[trnDropdownMenuItem],trn-dropdown-menu-item',
  hostDirectives: [
    {
...
@Directive({
  selector: '[trnDropdownMenuLabel],trn-dropdown-menu-label',
  host: {
...
export class TrnDropdownMenuRadioIndicator {
  constructor() {
    classes(
      () =>
...
/** @internal. Use TrnDropdownMenuRadio instead. */
@Directive({
  selector: '[trnDropdownMenuRadioCdk]',
  providers: [
    { provide: CdkMenuItemRadio, useExisting: TrnDropdownMenuRadioCdk },
    { provide: CdkMenuItemSelectable, useExisting: TrnDropdownMenuRadioCdk },
    { provide: CdkMenuItem, useExisting: CdkMenuItemSelectable },
  ],
})
export class TrnDropdownMenuRadioCdk extends CdkMenuItemRadio {
  public readonly keepOpen = input<boolean, BooleanInput>(true, { transform: booleanAttribute });
...
@Directive({
  ...
  hostDirectives: [
    {
      directive: TrnDropdownMenuRadioCdk,
      inputs: ['cdkMenuItemDisabled: disabled', 'cdkMenuItemChecked: checked', 'keepOpen'],
      outputs: ['cdkMenuItemTriggered: triggered'],
    },
    { directive: TrnDropdownMenuFocusOnHover, inputs: [], outputs: [] },
...
export class TrnDropdownMenuRadio {
  protected readonly _cdkMenuItem = inject(TrnDropdownMenuRadioCdk);
...
@Directive({
  selector: '[trnDropdownMenuSeparator],trn-dropdown-menu-separator',
...
@Directive({
  selector: '[trnDropdownMenuShortcut],trn-dropdown-menu-shortcut',
  host: { 'data-slot': 'dropdown-menu-shortcut' },
})
export class TrnDropdownMenuShortcut {
  constructor() {
...
@Directive({
  ...
  providers: [{ provide: MENU_SIDE, useExisting: forwardRef(() => TrnDropdownMenuSubTrigger) }],
  hostDirectives: [
    {
      directive: CdkMenuTrigger,
      inputs: ['cdkMenuTriggerFor: trnDropdownMenuSubTrigger', 'cdkMenuTriggerData: trnDropdownMenuTriggerData'],
      outputs: ['cdkMenuOpened: trnDropdownMenuSubOpened', 'cdkMenuClosed: trnDropdownMenuSubClosed'],
    }],
...
export class TrnDropdownMenuSubTrigger {
  ...
  private readonly _config = injectTrnDropdownMenuConfig();
   ...
   * overwrites that unconditionally. `TrnDropdownMenuSub` already believes the same thing — its
...
@Directive({
  selector: '[trnDropdownMenuSub],trn-dropdown-menu-sub',
  hostDirectives: [
    // CdkMenu declares one output, `closed`, and it stays internal deliberately. Closure is
    // already public on the TRIGGER as `trnDropdownMenuClosed` (and `…SubClosed`), which is
...
export interface TrnDropdownMenuConfig {
  align: MenuAlign;
  side: MenuSide;
}

const defaultConfig: TrnDropdownMenuConfig = {
  align: 'start',
  side: 'bottom',
};

const TrnDropdownMenuConfigToken = new InjectionToken<TrnDropdownMenuConfig>('TrnDropdownMenuConfig');

export function provideTrnDropdownMenuConfig(config: Partial<TrnDropdownMenuConfig>): ValueProvider {
  return { provide: TrnDropdownMenuConfigToken, useValue: { ...defaultConfig, ...config } };
}

export function injectTrnDropdownMenuConfig(): TrnDropdownMenuConfig {
  return inject(TrnDropdownMenuConfigToken, { optional: true }) ?? defaultConfig;
}

@Directive({
  selector: '[trnDropdownMenuTrigger]',
  providers: [{ provide: MENU_SIDE, useExisting: forwardRef(() => TrnDropdownMenuTrigger) }],
  hostDirectives: [
    {
      directive: CdkMenuTrigger,
      inputs: ['cdkMenuTriggerFor: trnDropdownMenuTrigger', 'cdkMenuTriggerData: trnDropdownMenuTriggerData'],
      outputs: ['cdkMenuOpened: trnDropdownMenuOpened', 'cdkMenuClosed: trnDropdownMenuClosed'],
    }],
...
@Directive({
  selector: '[trnDropdownMenu],trn-dropdown-menu',
  hostDirectives: [
    // CdkMenu declares one output, `closed`, and it stays internal deliberately. Closure is
    // already public on the TRIGGER as `trnDropdownMenuClosed` (and `…SubClosed`), which is
    // the element consumers bind to; re-exposing it here would give the same lifecycle two
