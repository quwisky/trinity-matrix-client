...
describe('RoomsPage space filtering', () => {
  ...
  it('Home (no space) shows only direct messages', async () => {
    const shell = build();
    shell.nav.onSelectSpace({ spaceId: null, accountId: '@me:hs' }); // click Home — leaves the default Recent view
  ...
  it('showing Rooms clears the active space', async () => {
    const shell = build();
    shell.nav.onSelectSpace({ spaceId: '!s:hs', accountId: '@me:hs' }); // a space is selected…
    await settleWorkspace();
  ...
  it('Home returns to direct messages from the Rooms view', async () => {
    ...
    shell.nav.onSelectSpace({ spaceId: null, accountId: '@me:hs' }); // clicking Home
    await settleWorkspace();
    expect(shell.store.roomsView()).toBe(false);
