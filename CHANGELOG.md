...
- **Menus, tooltips and progress bars now hold still on their own for reduced motion.** They
  already did, but only because a blanket rule elsewhere in the app was catching them; each
  now asks for it directly, so the behaviour survives the component being used anywhere else.
