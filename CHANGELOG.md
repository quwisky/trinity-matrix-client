...
- **See all your accounts' conversations in one place.** When more than one account is
  ...
  account, each marked with a small badge for the account it belongs to, so you can tell
  apart the same person reached through two of your accounts. Opening a conversation from
  another account switches to it automatically, so everything you do there (sending, receipts,
  reactions) happens as the right account. With one account signed in, nothing changes.
