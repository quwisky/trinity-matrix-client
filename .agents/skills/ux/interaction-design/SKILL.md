...
description: |
  ...
  DO NOT USE FOR: CSS animation syntax (use angular-developer),
  performance profiling (use diagnosing-bugs),
  WCAG audit (use playwright-best-practices)
...
function AnimatedModal({ children }: { children: React.ReactNode }) {
  ...
  return <div className={cn('transition-all', reduced ? 'duration-0' : 'duration-200 ease-out')}>{children}</div>;
}
  ...
  return (
    <div className="space-y-1.5">
      ...
      {React.cloneElement(children as React.ReactElement, {
        ...
        className: cn((children as React.ReactElement).props.className, error && 'border-destructive focus-visible:ring-destructive'),
      })}
      {error && (
...
- [Diagnosing bugs](../../diagnosing-bugs/SKILL.md) — measured feedback loops for performance regressions.
- [design-systems](../design-systems/SKILL.md)
