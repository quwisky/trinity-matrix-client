...
const UI_VENDOR_IMPORT_MESSAGE =
  'Only libs/kit/* (the vendored kit) and libs/ui may import a third-party UI package. Use the kit component, or @trinity/kit/overlay for dialogs and toasts — see docs/architecture/ui-and-theming.md.';
...
export default defineConfig([
  ...
  },
  {
    ...
    // widen this rule.
    // libs/kit is included even though it is generated: it is presentational UI that
    // must never reach the SDK, and leaving it out made this rule the one thing
    // libs/ui and libs/kit disagreed on — which scripts/lint-invariants.spec.mjs
  ...
  },
  {
    ...
    // class-string helpers in TS (trn()/cva()/classes()). Class ORDER is owned by
  ...
  {
    // libs/kit/* is canonical spartan-ng code generated by @spartan-ng/cli (we own it, but
    // don't author it). It still breaks two of our conventions — un-suffixed class names
    // (TrnButton, not TrnButtonDirective) and aliased inputs, including `class` — so those
    // two stay exempt rather than fighting the generator on every re-sync.
    //
    // The selector rules are NOT exempt any more, and that is the point of #150: once the
    // prefix is `trn`, the generated selectors satisfy our own rules, so `trn-card` and
    // `[trnBtn]` are checked here exactly as they are everywhere else. All 62 kit selectors
    // were linted against the real configs before the exemption was dropped — including the
    // awkward shapes (`[trnCard],trn-card`, `button[trnBtn], a[trnBtn]`, `img[trnAvatarImage]`,
    // `trn-progress,[trnProgress]`) — with zero errors. `lint-invariants.spec.mjs` asserts
    // the exemption is exactly these two, so a re-widening fails there.
    files: ['libs/kit/**/*.ts'],
    rules: {
