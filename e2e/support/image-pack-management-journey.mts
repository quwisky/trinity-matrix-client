...
export async function runImagePackManagementJourney({
  page,
  request,
  session,
  navigate,
  ...
  if (navigate) {
    ...
    await login(page, loginSession);
  }
  await openImagePackJourneyRoom(page, roomName);
  ...
  await sourceInput.pressSequentially(roomAlias);
  ...
  await verifyInstalledOnSecondClient?.({ hs, user, pass, roomName });
