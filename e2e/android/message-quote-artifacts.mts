...
export interface MessageQuotePublicationSafety {
...
export interface MessageQuoteSecretIds {
  readonly run: string;
  readonly account?: {
  ...
  /** The uploaded image's `mxc://` URI; its media id is registered on its own. */
 ...
 */
export function messageQuoteSecrets(
  stage: MessageQuoteStageId,
  ids: MessageQuoteSecretIds,
): Record<string, string> {
  assert(MESSAGE_QUOTE_STAGES.some((entry) => entry.id === stage),
    'Message-quote secrets belong to a contract stage');
  const prefix = `SECRET_QUOTE_${stage.toUpperCase().replaceAll('-', '_')}`;
  const values: Record<string, string> = {};
  const add = (key: string, value: string | undefined): void => {
    if (value === undefined) return;
    assert(typeof value === 'string' && value.length > 0,
      'Message-quote secret values are non-empty strings');
  ...
  if (ids.contentUri !== undefined) {
    const media = /^mxc:\/\/[^/]+\/(.+)$/u.exec(ids.contentUri);
    assert(media, 'The image content URI is an MXC URI with a media id');
    addEncoded('MEDIA_URI', ids.contentUri);
    add('MEDIA_ID', media[1]);
  }
  ids.eventIds?.forEach((eventId, index) => addEncoded(`EVENT_${index + 1}_ID`, eventId));
  for (const value of Object.values(values))
    assert(value !== 'localhost', 'The bare server name is never a message-quote secret');
  return values;
}
...
export async function scrubMessageQuoteArtifacts(
  output: string,
  secrets: Readonly<Record<string, string>>,
): Promise<void> {
  ...
  const scrub = async (directory: string): Promise<void> => {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) await scrub(path);
        ...
        assert(entry.isFile(), 'Message-quote diagnostic is a regular file');
...
export async function scanMessageQuoteArtifacts(
  output: string,
  secrets: Readonly<Record<string, string>>,
): Promise<void> {
  ...
  const scan = async (directory: string): Promise<void> => {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      assert(!raster.has(extname(path).toLowerCase()),
        'No raster proof in message-quote diagnostics');
      assert(entry.isFile() &&
        (entry.name === 'publication-safe' ||
          publishableText.has(extname(path).toLowerCase())),
      'Message-quote diagnostics contain only recognized text files');
      const value = await readFile(path, 'utf8');
      for (const secret of values)
        assert(!value.includes(secret), 'No raw credential or identifier in message-quote diagnostics');
      for (const pattern of encoded)
        assert(!pattern.test(value), 'No URL-encoded credential or identifier in message-quote diagnostics');
      assert(!/syt_|\bBearer\s+\S+/iu.test(value),
        'Authorization absent from message-quote diagnostics');
      assert(!/access_token["']?\s*[:=]\s*["']?[\w.~-]{8,}/iu.test(value),
        'Media and query-string access tokens absent from message-quote diagnostics');
      assert(!/<(?:map\b|string\b)[^>]*>/iu.test(value),
        'Raw native Preferences XML absent from message-quote diagnostics');
...
async function requireTextFile(path: string): Promise<void> {
  ...
  assert(metadata.isFile() && metadata.size > 0,
    'Required message-quote text capture or provenance exists');
}
...
async function readJsonObject(path: string): Promise<Record<string, unknown>> {
  ...
  assert(typeof value === 'object' && value !== null && !Array.isArray(value),
    'Message-quote diagnostic JSON is an object');
  return value as Record<string, unknown>;
}

function assertPassedReport(report: Record<string, unknown>): void {
  assert(report['status'] === 'passed' &&
    report['expectedStages'] === MESSAGE_QUOTE_STAGES.length &&
    report['expectedAssertionRecords'] === MESSAGE_QUOTE_ASSERTION_RECORDS &&
    report['attempt'] === 1 && report['retries'] === 0,
  'Message-quote report has every successful unretried stage');
  assert(Array.isArray(report['stages']) &&
    report['stages'].length === MESSAGE_QUOTE_STAGES.length,
  'Message-quote report has exactly the contract stages');
  const identities = new Set<unknown>();
  MESSAGE_QUOTE_STAGES.forEach((entry, index) => {
    const stage = (report['stages'] as unknown[])[index] as Record<string, unknown>;
    assert(typeof stage === 'object' && stage !== null,
      'Message-quote stage report is an object');
    ...
    `Message-quote stage ${index + 1} is ${entry.id} with every record passed`);
    assert.deepEqual(stage['assertions'], [...entry.assertions],
      `Message-quote ${entry.id} parity identities are complete and source ordered`);
  ...
  assert.equal(identities.size, MESSAGE_QUOTE_ASSERTION_RECORDS,
    'Message-quote parity identities are unique across stages');
}

/** Only a complete, captured, cleaned two-stage run may receive the hosted upload marker. */
export async function markMessageQuoteDiagnosticsSafe(
  output: string,
  secrets: Readonly<Record<string, string>>,
  flags: Readonly<MessageQuotePublicationSafety>,
  ...
  try {
    ...
    assert(!flags.unsafeSecrets && !flags.cleanupFailed && !flags.scrubFailed,
      'Incomplete secrets, cleanup or scrub blocks message-quote publication');
    ...
    assert.deepEqual(await readJsonObject(join(output, 'journeys.json')), current,
      'Persisted message-quote report matches the run');
    ...
    assert(provenance['schemaVersion'] === 1 && typeof installed === 'object' && installed !== null,
      'Message-quote runtime provenance records its profile');
    assert.deepEqual(installed, {
      requested: PIXEL_5_ACCOUNT_PROFILE,
      digest: digest(JSON.stringify(PIXEL_5_ACCOUNT_PROFILE)),
    }, 'Message-quote runtime provenance used the Pixel 5 profile');
    for (const entry of MESSAGE_QUOTE_STAGES) {
      ...
      assert.deepEqual(applied['requested'], PIXEL_5_ACCOUNT_PROFILE,
        `Message-quote ${entry.id} applied the Pixel 5 profile`);
      for (const name of PASSED_CAPTURES)
        await requireTextFile(join(output, entry.id, name));
    }
    await scanMessageQuoteArtifacts(output, secrets);
    signal?.throwIfAborted();
...
export async function runMessageQuoteStageCleanup(
...
export async function revokeMessageQuotePublicationOnAbort(
  output: string,
  report: AbortReport,
  signal: AbortSignal,
): Promise<void> {
  ...
  if (stage) stage.error = 'Cancelled before message-quote publication';
