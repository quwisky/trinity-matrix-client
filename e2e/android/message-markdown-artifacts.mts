...
export interface MessageMarkdownPublicationSafety {
...
export interface MessageMarkdownSecretIds {
  readonly run: string;
  readonly account?: {
 ...
 */
export function messageMarkdownSecrets(
  stage: MessageMarkdownStageId,
  ids: MessageMarkdownSecretIds,
): Record<string, string> {
  assert(MESSAGE_MARKDOWN_STAGES.some((entry) => entry.id === stage),
    'Message-markdown secrets belong to a contract stage');
  const prefix = `SECRET_MARKDOWN_${stage.toUpperCase().replaceAll('-', '_')}`;
  const values: Record<string, string> = {};
  const add = (key: string, value: string | undefined): void => {
    if (value === undefined) return;
    assert(typeof value === 'string' && value.length > 0,
      'Message-markdown secret values are non-empty strings');
  ...
  for (const value of Object.values(values))
    assert(value !== 'localhost', 'The bare server name is never a message-markdown secret');
  return values;
}
...
export async function scrubMessageMarkdownArtifacts(
  output: string,
  secrets: Readonly<Record<string, string>>,
): Promise<void> {
  ...
  const scrub = async (directory: string): Promise<void> => {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) await scrub(path);
        ...
        assert(entry.isFile(), 'Message-markdown diagnostic is a regular file');
...
export async function scanMessageMarkdownArtifacts(
  output: string,
  secrets: Readonly<Record<string, string>>,
): Promise<void> {
  ...
  const scan = async (directory: string): Promise<void> => {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      assert(!raster.has(extname(path).toLowerCase()),
        'No raster proof in message-markdown diagnostics');
      assert(entry.isFile() &&
        (entry.name === 'publication-safe' ||
          publishableText.has(extname(path).toLowerCase())),
      'Message-markdown diagnostics contain only recognized text files');
      const value = await readFile(path, 'utf8');
      for (const secret of values)
        assert(!value.includes(secret), 'No raw credential or identifier in message-markdown diagnostics');
      for (const pattern of encoded)
        assert(!pattern.test(value), 'No URL-encoded credential or identifier in message-markdown diagnostics');
      assert(!/syt_|\bBearer\s+\S+/iu.test(value),
        'Authorization absent from message-markdown diagnostics');
      assert(!/<(?:map\b|string\b)[^>]*>/iu.test(value),
        'Raw native Preferences XML absent from message-markdown diagnostics');
...
async function requireTextFile(path: string): Promise<void> {
  ...
  assert(metadata.isFile() && metadata.size > 0,
    'Required message-markdown text capture or provenance exists');
}
...
async function readJsonObject(path: string): Promise<Record<string, unknown>> {
  ...
  assert(typeof value === 'object' && value !== null && !Array.isArray(value),
    'Message-markdown diagnostic JSON is an object');
  return value as Record<string, unknown>;
}

function assertPassedReport(report: Record<string, unknown>): void {
  assert(report['status'] === 'passed' &&
    report['expectedStages'] === MESSAGE_MARKDOWN_STAGES.length &&
    report['expectedAssertionRecords'] === MESSAGE_MARKDOWN_ASSERTION_RECORDS &&
    report['attempt'] === 1 && report['retries'] === 0,
  'Message-markdown report has three successful unretried stages');
  assert(Array.isArray(report['stages']) &&
    report['stages'].length === MESSAGE_MARKDOWN_STAGES.length,
  'Message-markdown report has exactly the contract stages');
  const identities = new Set<unknown>();
  MESSAGE_MARKDOWN_STAGES.forEach((entry, index) => {
    const stage = (report['stages'] as unknown[])[index] as Record<string, unknown>;
    assert(typeof stage === 'object' && stage !== null,
      'Message-markdown stage report is an object');
    ...
    `Message-markdown stage ${index + 1} is ${entry.id} with every record passed`);
    assert.deepEqual(stage['assertions'], [...entry.assertions],
      `Message-markdown ${entry.id} parity identities are complete and source ordered`);
  ...
  assert.equal(identities.size, MESSAGE_MARKDOWN_ASSERTION_RECORDS,
    'Message-markdown parity identities are unique across stages');
}

/** Only a complete, captured, cleaned three-stage run may receive the hosted upload marker. */
export async function markMessageMarkdownDiagnosticsSafe(
  output: string,
  secrets: Readonly<Record<string, string>>,
  flags: Readonly<MessageMarkdownPublicationSafety>,
  ...
  try {
    ...
    assert(!flags.unsafeSecrets && !flags.cleanupFailed && !flags.scrubFailed,
      'Incomplete secrets, cleanup or scrub blocks message-markdown publication');
    ...
    assert.deepEqual(await readJsonObject(join(output, 'journeys.json')), current,
      'Persisted message-markdown report matches the run');
    ...
    assert(provenance['schemaVersion'] === 1 && typeof installed === 'object' && installed !== null,
      'Message-markdown runtime provenance records its profile');
    assert.deepEqual(installed, {
      requested: PIXEL_5_ACCOUNT_PROFILE,
      digest: digest(JSON.stringify(PIXEL_5_ACCOUNT_PROFILE)),
    }, 'Message-markdown runtime provenance used the Pixel 5 profile');
    for (const entry of MESSAGE_MARKDOWN_STAGES) {
      ...
      assert.deepEqual(applied['requested'], PIXEL_5_ACCOUNT_PROFILE,
        `Message-markdown ${entry.id} applied the Pixel 5 profile`);
      for (const name of PASSED_CAPTURES)
        await requireTextFile(join(output, entry.id, name));
    }
    await scanMessageMarkdownArtifacts(output, secrets);
    signal?.throwIfAborted();
...
export async function runMessageMarkdownStageCleanup(
...
export async function revokeMessageMarkdownPublicationOnAbort(
  output: string,
  report: AbortReport,
  signal: AbortSignal,
): Promise<void> {
  ...
  if (stage) stage.error = 'Cancelled before message-markdown publication';
