 ...
 * expected values to its message even when a custom message is given, and an
 * observed value may be an identifier that was never registered, so an
...
export function redactStageFailure(
  ...
  return new Error(
    redactDiagnosticText(`Android message-spoiler ${stageId} failed\n${lines.join('\n')}`, secrets),
  );
}
