...
describe('CI execution contract', () => {
  ...
  it('runs Space leave after core Space Settings and before retained Playwright on shard 2', () => {
    const script = workflow.jobs['android-e2e'].steps.find(
      (step) => step.id === 'android',
    ).with.script;
    ...
    expect(playwright).toBeGreaterThan(spaceLeave);
