...
/**
 * The TWO rules `libs/kit/*` is exempt from, because that code is generated by
 * `@spartan-ng/cli` and owned upstream: un-suffixed class names and aliased inputs.
 *
 * It was four until #150. Renaming the generator's prefix to `trn` made its selectors
 * satisfy our own rules, so `component-selector` and `directive-selector` are enforced in
 * the kit now like everywhere else — which is half the point of that rebrand. The diff
 * assertion below is what proves the exemption shrank rather than being quietly re-widened.
 */
const KIT_EXEMPT_RULES = [
  '@angular-eslint/no-input-rename',
  '@angular-eslint/component-class-suffix',
];

describe('lint invariants', () => {
  it('exempts libs/kit from exactly the two generated-code rules, and nothing else', async () => {
    const app = await resolve('libs/ui/src/index.ts');
    const spartan = await resolve('libs/kit/tooltip/src/lib/trn-tooltip.ts');

    for (const ruleId of KIT_EXEMPT_RULES) {
      expect(
        severityOf(app, ruleId),
        `${ruleId} must be an error outside the kit`,
      ).toBe(2);
      expect(
        severityOf(spartan, ruleId),
        `${ruleId} must be off inside the kit`,
      ).toBe(0);
    }
    ...
    expect(differing.sort()).toEqual([...KIT_EXEMPT_RULES].sort());
  });
...
describe('UI vendor boundary', () => {
  ...
  it('leaves the vendored kit itself free to import its own vendor', async () => {
    const kit = await resolve('libs/kit/tooltip/src/lib/trn-tooltip.ts');
    const constraints =
