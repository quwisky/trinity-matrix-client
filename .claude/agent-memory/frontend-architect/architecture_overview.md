...
Library layout (`libs/`):
- `core` (`@trinity/core`, tags `type:feature`... actually no tags shown; import path in tsconfig.base.json) — all matrix-js-sdk wrapping. Services are `@Injectable({providedIn:'root'})`, expose read-only `signal()`s + an idempotent `connect()` that bridges SDK events → signals, and cold `Observable`s for one-shot actions (errors thrown as `Error` with user-facing messages). Examples: `MatrixClientService`, `RoomsService`, `TimelineService`, `AuthService`, `CryptoService`, `SecretStorageKeyService`. Everything re-exported from `libs/core/src/index.ts`.
- `feature-auth` (`@trinity/feature-auth`) — pages behind `authGuard`. NO tests (no vite.config/test-setup/tsconfig.spec).
- `feature-rooms` (`@trinity/feature-rooms`) — the authed shell; HAS the full Vitest setup (vite.config.ts, src/test-setup.ts, tsconfig.spec.json, test target in project.json).
- All feature libs: `projectType: library`, `prefix: trn`, tags `["type:feature","scope:trinity"]`, only `tsconfig.json` (lib) + `tsconfig.spec.json` (no tsconfig.lib.json). Path aliases live in `tsconfig.base.json`.
